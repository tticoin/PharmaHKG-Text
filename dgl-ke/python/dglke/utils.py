...
def create_dicts(dataset):
    map_head_d = {'interaction':'DRUGBANK', 'category':'DRUGBANK',
                  'atc':'DRUGBANK', 'atc-hierarchy':'ATC', 'pathway':'DRUGBANKUNIPROT',
                  'targets':'DRUGBANK', 'enzymes':'DRUGBANK', 'carriers':'DRUGBANK', 'transporters':'DRUGBANK'}
    map_tail_d = {'interaction':'DRUGBANK', 'category':'MESH',
                  'atc':'ATC', 'atc-hierarchy':'ATC', 'pathway':'SMPDB',
                  'targets':'UNIPROT', 'enzymes':'UNIPROT', 'carriers':'UNIPROT', 'transporters':'UNIPROT'}

    # Node ids to entity
    nid_to_entity = {v for k,v in dataset.entity2id.items()}
    # Edge ids to relation
    eid_to_relation = {v for k,v in dataset.relation2id.items()}

    # Parent: ALL node ids
    # Sub: No textual node ids
    nid_parent_to_sub = {}
    nid_sub_to_parent = []

    # Dict of node id sets (key == node type)
    nids_for_type = {}
    nids_for_type['DRUGBANKUNIPROT'] = set()
    
    for k, v in dataset.entity2id.items():
        if len(k.split('::')) == 2:
            nid_parent_to_sub[v] = len(nid_parent_to_sub)
            nid_sub_to_parent.append(v)
            node_type = k.split('::')[0]
        elif len(k.split('::')) == 3:
            rel = k.split('::')[2]
            node_type = 'TEXTnode' + k.split('::')[2]
            # Textual nodes
            if node_type not in map_head_d: map_head_d[rel] = 'NotTEXTnode'
            if node_type not in map_tail_d: map_tail_d[rel] = node_type

        if node_type not in nids_for_type: nids_for_type[node_type] = set()
        nids_for_type[node_type].add(v)

        # For Pathway
        if node_type in 'DRUGBANKUNIPROT': nids_for_type['DRUGBANKUNIPROT'].add(v)

    notext_nids = set()
    for k, v in nids_for_type.items():
        if not 'TEXTnode' in k:
            notext_nids = notext_nids | v
    nids_for_type['NotTEXTnode'] = notext_nids

    # Preparing dicts for hetero-graph negative sampling
    neg_head_d = {}
    neg_tail_d = {}
    neg_eval_head_d = {}
    neg_eval_tail_d = {}
    for k, v in dataset.relation2id.items():
        if v not in neg_head_d: neg_head_d[v] = set()
        if v not in neg_tail_d: neg_tail_d[v] = set()
        neg_head_d[v] = nids_for_type[map_head_d[k]]
        neg_tail_d[v] = nids_for_type[map_tail_d[k]]

        if v not in neg_eval_head_d: neg_eval_head_d[v] = np.ones(len(nid_parent_to_sub), dtype='i')
        if v not in neg_eval_tail_d: neg_eval_tail_d[v] = np.ones(len(nid_parent_to_sub), dtype='i')
        for nid in nids_for_type[map_head_d[k]]:
            if nid in nid_parent_to_sub: neg_eval_head_d[v][nid_parent_to_sub[nid]] = 0
        for nid in nids_for_type[map_tail_d[k]]:
            if nid in nid_parent_to_sub: neg_eval_tail_d[v][nid_parent_to_sub[nid]] = 0

    return nid_parent_to_sub, nid_sub_to_parent, neg_head_d, neg_tail_d, neg_eval_head_d, neg_eval_tail_d
...
class CommonArgParser(argparse.ArgumentParser):
    def __init__(self):
        ...
        self.add_argument('--batch_size', type=int, default=4096,
                          help='The batch size for training.')
        self.add_argument('--batch_size_eval', type=int, default=4096,
        ...
        self.add_argument('-rc', '--regularization_coef', type=float, default=1e-9,
        ...
        self.add_argument('--negative_sampling_for_hetero', action='store_true', help='Sampling negative triples by considering the types nodes and edges')
