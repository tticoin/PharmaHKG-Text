...
from utils import get_compatible_batch_size, save_model, CommonArgParser, create_dicts
...
class ArgParser(CommonArgParser):
    def __init__(self):
        ...
        self.add_argument('--eval_interval_epoch', type=int, default=1)

        self.add_argument('--entity_emb_file', type=str, default=None)
        self.add_argument('--relation_emb_file', type=str, default=None)

        self.add_argument('--do_alignment', action='store_true', help='')
        self.add_argument('--alignment_reg_coef', type=float, default=1e-9, help='')
...
def main():
    ...
    nid_p2s, nid_s2p, neg_head_d, neg_tail_d, neg_eval_head_d, neg_eval_tail_d = create_dicts(dataset)
    if not args.negative_sampling_for_hetero:
        neg_head_d, neg_tail_d, neg_eval_head_d, neg_eval_tail_d = None, None, None, None

    if args.neg_sample_size_eval < 0:
        #args.neg_sample_size_eval = dataset.n_entities
        args.neg_sample_size_eval = len(nid_p2s)
    ...
    #foo(train_data, nids_d)
    args.num_train = train_data.num_train
    ...
    if args.valid:
        if args.num_proc > 1:
            ...
            valid_sampler_head = eval_dataset.create_sampler('valid', args.batch_size_eval,
                                                             ...
                                                             mode='chunk-head',
                                                             num_workers=args.num_workers,
                                                             rank=0, ranks=1, neg_d=neg_eval_head_d)
            valid_sampler_tail = eval_dataset.create_sampler('valid', args.batch_size_eval,
                                                             args.neg_sample_size_eval,
                                                             args.neg_sample_size_eval,
                                                             args.eval_filter,
                                                             mode='chunk-tail',
                                                             num_workers=args.num_workers,
                                                             rank=0, ranks=1, neg_d=neg_eval_tail_d)
    ...
    if args.entity_emb_file is not None:
        if args.do_alignment:
            model.bert_emb.load(args.data_path, args.entity_emb_file)
        else:
            model.entity_emb.load(args.data_path, args.entity_emb_file)
    if args.relation_emb_file is not None:
        model.entity_emb.load(args.data_path, args.relation_emb_file)
    ...
    if args.num_proc > 1:
        ...
        train(args, model, train_sampler, valid_samplers, rel_parts=rel_parts, nt_idx=nid_s2p)
    ...
    if args.test:
        ...
        if args.num_test_proc > 1:
            ...
            #pos_scores, neg_scores, mask = test(args, model, [test_sampler_head, test_sampler_tail], nt_idx=nid_s2p)
