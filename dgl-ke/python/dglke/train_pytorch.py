...
def train(args, model, train_sampler, valid_samplers=None, rank=0, rel_parts=None, cross_rels=None, barrier=None, client=None, nt_idx=None):
    logs = []
    ...
    forward_time = 0
    backward_time = 0
    args.max_step = args.num_epochs * args.num_train // args.batch_size
    args.eval_interval = args.eval_interval_epoch * args.num_train // args.batch_size
    ...
    for step in range(0, args.max_step):
        ...
        if args.valid and (step + 1) % args.eval_interval == 0 and step > 1 and valid_samplers is not None:
            ...
            test(args, model, valid_samplers, rank, mode='Valid', nt_idx=nt_idx)
...
def test(args, model, test_samplers, rank=0, mode='Test', queue=None, nt_idx=None):
    if len(args.gpu) > 0:
        gpu_id = args.gpu[rank % len(args.gpu)] if args.mix_cpu_gpu and args.num_proc > 1 else args.gpu[0]
    else:
        gpu_id = -1
    ...
    with th.no_grad():
        ...
        for sampler in test_samplers:
            for pos_g, neg_g in sampler:
                ...
                model.forward_test(pos_g, neg_g, logs, gpu_id, nt_idx)
            #    start=time.time()
            #    pos_scores, neg_scores, mask = model.forward_test(pos_g, neg_g, logs, gpu_id, nt_idx)
            #    if all_pos_scores is None:
            #        all_pos_scores = pos_scores
            #        all_neg_scores = neg_scores
            #        all_mask = mask
            #    else:
            #        all_pos_scores = th.cat((all_pos_scores, pos_scores), dim=0)
            #        all_neg_scores = th.cat((all_neg_scores, neg_scores), dim=0)
            #        all_mask = th.cat((all_mask, neg_scores), dim=0)
            #    print('concat time', time.time()-start)
            #print(all_pos_scores.size(), all_neg_scores.size())
        ...
        if queue is not None:
            ...
            for k, v in metrics.items():
                ...
                if not args.test:
                    with open(os.path.join(args.save_path, 'results.txt'), 'a') as f:
                        print('[{}]{} average {}: {}'.format(rank, mode, k, v), file=f)
    test_samplers[0] = test_samplers[0].reset()
    test_samplers[1] = test_samplers[1].reset()

    #return all_pos_scores, all_neg_scores, all_mask
