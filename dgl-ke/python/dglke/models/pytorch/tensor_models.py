...
class ExternalEmbedding:
    ...
    def __call__(self, idx, gpu_id=-1, trace=True, nt_idx=None):
    ...
    def curr_emb_(self, trace, gpu_id):
        embs = []
        for idx, _ in trace:
            embs.append(self.emb[idx])
        return th.cat(embs, 0).cuda(gpu_id)
    ...
    def load(self, path, name):
        ...
        print('Model emb size:{}  NPY emb size:{}'.format(self.emb.shape, npy_file.shape))
        assert self.emb.shape == npy_file.shape
        self.emb = th.Tensor(npy_file)
