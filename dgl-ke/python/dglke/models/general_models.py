...
class KEModel(object):
    ...
    def __init__(self, args, model_name, n_entities, n_relations, hidden_dim, gamma,
        ...
        self.bert_emb = ExternalEmbedding(args, n_entities, entity_dim,
                                            F.cpu() if args.mix_cpu_gpu else device)
    ...
    def predict_neg_score(self, pos_g, neg_g, to_device=None, gpu_id=-1, trace=False,
                          neg_deg_sample=False, nt_idx=None):
        ...
        #print('chunk_size', chunk_size, 'num_chunks', num_chunks, 'neg_sample_size', neg_sample_size)
        ...
        if neg_g.neg_head:
            ...
            neg_head_ids = neg_g.ndata['id'][neg_g.new_head_nid]
            #neg_head = self.entity_emb(neg_head_ids, gpu_id, trace)
            neg_head = self.entity_emb(neg_head_ids, gpu_id, trace, nt_idx)
            ...
            neg_tail_ids = neg_g.ndata['id'][neg_g.new_tail_nid]
            #neg_tail = self.entity_emb(neg_tail_ids, gpu_id, trace)
            neg_tail = self.entity_emb(neg_tail_ids, gpu_id, trace, nt_idx)
            head_ids, tail_ids = pos_g.all_edges(order='eid')
            if to_device is not None and gpu_id >= 0:
    ...
    def forward_test(self, pos_g, neg_g, logs, gpu_id=-1, nt_idx=None):
        ...
        pos_g.ndata['emb'] = self.entity_emb(pos_g.ndata['id'], gpu_id, False, nt_idx)
    ...
    def forward(self, pos_g, neg_g, gpu_id=-1):
        ...
        #bert_emb = self.bert_emb(pos_g.ndata['id'], gpu_id, True)
        ...
        if gpu_id >= 0:
            neg_score = self.predict_neg_score(pos_g, neg_g, to_device=None,
        ...
        if self.args.do_alignment:
            coef, nm = self.args.alignment_reg_coef, self.args.regularization_norm
            a_reg = coef * norm(self.entity_emb.curr_emb() - self.bert_emb.curr_emb_(self.entity_emb.trace, gpu_id), nm)
            log['alignment_reg'] = get_scalar(a_reg)
            loss = loss + a_reg
